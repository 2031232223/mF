...
class _SettingsPageState extends State<SettingsPage> {
  ...
  void _showInputDialog(String title, String currentValue, Function(String) onSave) {
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(backgroundColor: const Color(0xFF1E1E1E), title: Text(title, style: const TextStyle(color: Colors.green)), content: TextField(controller: controller, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(hintText: 'Ingresa el valor', border: OutlineInputBorder(), hintStyle: TextStyle(color: Colors.grey))), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancelar', style: TextStyle(color: Colors.grey))), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.green), onPressed: () { onSave(controller.text); Navigator.pop(ctx); }, child: const Text('Guardar', style: TextStyle(color: Colors.white))),],));
  }
  ...
  void _showRestoreDialog() { showDialog(context: context, builder: (ctx) => AlertDialog(backgroundColor: const Color(0xFF1E1E1E), title: const Text('Restaurar Base de Datos', style: TextStyle(color: Colors.green)), content: const Text('¿Estás seguro?', style: TextStyle(color: Colors.grey)), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancelar', style: TextStyle(color: Colors.grey))), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red), onPressed: () { Navigator.pop(ctx); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('🔄 Restaurando...'), backgroundColor: Colors.blue)); }, child: const Text('Restaurar', style: TextStyle(color: Colors.white))),],)); }
  Future<void> _exportToCsv() async { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('📤 Exportando...'), backgroundColor: Colors.blue)); await Future.delayed(const Duration(seconds: 2)); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('✅ Exportado'), backgroundColor: Colors.green)); }
