...
class _CartPageState extends State<CartPage> {
  ...
  void _showCustomerSelector() {
    showModalBottomSheet(context: context, backgroundColor: const Color(0xFF1E1E1E), builder: (ctx) => Container(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const Text('Seleccionar Cliente', style: TextStyle(color: Colors.green, fontSize: 18, fontWeight: FontWeight.w600)), const Spacer(), IconButton(icon: const Icon(Icons.close, color: Colors.grey), onPressed: () => Navigator.pop(ctx)),]), const SizedBox(height: 16), if (_customers.isEmpty) const Text('No hay clientes', style: TextStyle(color: Colors.grey)), Expanded(child: ListView.builder(shrinkWrap: true, itemCount: _customers.length, itemBuilder: (context, index) { final c = _customers[index]; return Card(color: Colors.grey[900], margin: const EdgeInsets.only(bottom: 8), child: ListTile(leading: CircleAvatar(backgroundColor: Colors.green, child: Text(c.nombre[0], style: const TextStyle(color: Colors.white))), title: Text(c.nombre, style: const TextStyle(color: Colors.green)), subtitle: Text(c.telefono ?? '', style: TextStyle(color: Colors.grey[400])), onTap: () { setState(() => _selectedCustomer = c); Navigator.pop(ctx); },)); })),],)));
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF1E1E1E), appBar: AppBar(title: const Text('Carrito', style: TextStyle(color: Colors.green, fontWeight: FontWeight.w600)), backgroundColor: const Color(0xFF1E1E1E), elevation: 0, actions: [DropdownButton<String>(value: widget.selectedCurrency, dropdownColor: const Color(0xFF1E1E1E), underline: const SizedBox(), items: ['CUP','USD','MLC'].map((c) => DropdownMenuItem(value: c, child: Text(c, style: const TextStyle(color: Colors.green)))).toList(), onChanged: null), IconButton(icon: const Icon(Icons.close, color: Colors.green), onPressed: () => Navigator.pop(context)),]), body: Column(children: [
      Container(padding: const EdgeInsets.all(16), color: const Color(0xFF1E1E1E), child: Row(children: [const Text('Cliente:', style: TextStyle(fontWeight: FontWeight.w600, color: Colors.green)), const Spacer(), TextButton.icon(icon: const Icon(Icons.person_add, color: Colors.green), label: const Text('Nuevo', style: TextStyle(color: Colors.green, fontWeight: FontWeight.w600)), onPressed: () {},),])),
      Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), color: const Color(0xFF1E1E1E), child: Card(color: Colors.grey[900], child: ListTile(leading: CircleAvatar(backgroundColor: Colors.green, child: Icon(_selectedCustomer == null ? Icons.person : Icons.check, color: Colors.white)), title: Text(_selectedCustomer?.nombre ?? 'Seleccionar cliente', style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.green)), trailing: const Icon(Icons.chevron_right, color: Colors.green), onTap: _showCustomerSelector,))),
      Expanded(child: ListView.builder(padding: const EdgeInsets.all(16), itemCount: widget.cart.length, itemBuilder: (context, index) { final item = widget.cart[index]; final price = widget.selectedCurrency == 'CUP' ? item.precioCUP : item.precioCUP / widget.exchangeRate; return Card(margin: const EdgeInsets.only(bottom: 12), color: Colors.grey[900], elevation: 2, child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [CircleAvatar(backgroundColor: Colors.green, radius: 24, child: Text('${index+1}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.nombre, style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.green)), Text('${price.toStringAsFixed(2)} c/u', style: TextStyle(color: Colors.grey[400], fontSize: 12)),])), Row(children: [IconButton(icon: const Icon(Icons.remove_circle, color: Colors.red), onPressed: () { if (item.cantidad > 1) setState(() => item.cantidad--); }), Text('${item.cantidad}', style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.white, fontSize: 16)), IconButton(icon: const Icon(Icons.add_circle, color: Colors.green), onPressed: () { if (item.cantidad < item.stockDisponible) setState(() => item.cantidad++); }), IconButton(icon: const Icon(Icons.delete_forever, color: Colors.red), onPressed: () { setState(() => widget.cart.removeAt(index)); }),],)],))); })),
      Container(padding: const EdgeInsets.all(16), color: const Color(0xFF1E1E1E), child: Card(color: Colors.grey[900], child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [const Icon(Icons.local_offer, color: Colors.green), const SizedBox(width: 12), const Text('Descuento Global', style: TextStyle(fontWeight: FontWeight.w600, color: Colors.green)), const Spacer(), Switch(value: _discount > 0, onChanged: (v) { setState(() => _discount = v ? 10.0 : 0.0); }), activeColor: Colors.green,]),))),
      Container(padding: const EdgeInsets.all(16), color: Colors.grey[900], child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Subtotal:', style: TextStyle(color: Colors.grey)), Text('${_subtotal.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.green)),]), const SizedBox(height: 8), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('TOTAL:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.green)), Text('${_total.toStringAsFixed(2)} ${widget.selectedCurrency}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.green)),]), const Divider(height: 24, color: Colors.grey), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Pagado:', style: TextStyle(color: Colors.grey)), SizedBox(width: 150, child: TextField(controller: _paymentController, keyboardType: TextInputType.number, decoration: const InputDecoration(hintText: '0.00', border: InputBorder.none, contentPadding: EdgeInsets.symmetric(vertical: 8)), textAlign: TextAlign.right, style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.green),)),]), if (_paid < _total) Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Row(children: [Icon(Icons.warning, color: Colors.orange, size: 16), SizedBox(width: 4), Text('Faltante:', style: TextStyle(color: Colors.orange, fontWeight: FontWeight.w600)),]), Text('${(_total - _paid).toStringAsFixed(2)}', style: const TextStyle(color: Colors.orange, fontWeight: FontWeight.bold)),]), if (_paid >= _total && _paid > 0) Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Row(children: [Icon(Icons.check_circle, color: Colors.green, size: 16), SizedBox(width: 4), Text('Cambio:', style: TextStyle(color: Colors.green, fontWeight: FontWeight.w600)),]), Text('${_change.toStringAsFixed(2)}', style: const TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),]),])),
